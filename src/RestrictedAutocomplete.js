...
export class RestrictedAutocomplete {
  ...
  constructor(textField, callback) {
    ...
    try {
      if (!this.textField)
        throw ReferenceError("autocomplete textField is null");
      if (this.textField.type != "text")
        throw TypeError("autocomplete textField is not text");
      ...
      this.textField.addEventListener("focus", (e) => this.onFocus(e), false);
  ...
  setGoogleServices() {
    try {
      ...
      console.error(
        "Houve uma tentativa de conexão ao Google, porém não foi possível"
      );
    }
  }
  ...
  addrSelected(placeId) {
    ...
    let callBackError =
      "A função passada como parâmetro ao autocomplete " +
      "para ser executada após selecionado o endereço " +
      "apresentou um erro: ";

    this.getPlaceDetail(request)
      .then(
        (place) => this.setLastResult(place),
        (status) => {
  ...
  queryGooglePlacesAndShowResults() {
    ...
    this.dropDownPredictionList.setAttribute(
      "id",
      ...
      .catch((status) => {
        if (this.textField.value && this.textField.value != "") {
          if (status == google.maps.places.PlacesServiceStatus.ZERO_RESULTS)
            ...
            this.handleGoogleConetionError(
              "erro ao buscar " + "predição no google"
            );
        }
      })
      .finally(() => {
  ...
  showPredictions(places) {
    places.forEach((item) => {
      ...
      b.innerHTML =
        "<strong>" + addr.substr(0, this.textField.value.length) + "</strong>";
  ...
  showNoResutlsMsg() {
    let b = document.createElement("DIV");
    b.innerHTML =
      "<strong>" + "Não corresponde a nenhum endereço" + "</strong>";
  ...
  showGoogleCredits() {
    ...
    b.innerHTML =
      '<p class="poweredByGoogle">powered by ' +
  ...
  onTextChange(e) {
    ...
    wait(300)
      ...
      .catch((e) =>
        console.error(
          "Ocorreu um problema ao fazer " + "busca no Google: " + e.message
        )
      );
  }
  ...
  onFocus(e) {
    /**
     * if user doesn't click on google sugestion
     * and come back to type text after a while this
     * will force the component to refresh the token,
     * because if it try to reuse the last token and it
     * was invalidated by Google because inactivity
     * they will bill "per request"
     */
    this.placesSessionToken = null;
  }
